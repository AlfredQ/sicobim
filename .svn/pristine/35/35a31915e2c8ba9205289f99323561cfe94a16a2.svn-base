...
switch ($opc)
{
	...
	case 0:
		...
		$intTotalConsulta=$objMantenimiento->ConsultarDependencia(1);
		$_SESSION['contadorRegistro'] = $intTotalConsulta;			
		$intTotalConsulta=$objMantenimiento->ConsultarDependencia(0);
	...
	/* Cargar vista Consultar Activos Reales */
	case 8:
	break;
	...
	case 17:
		...
		$mantenimiento->setCodigoSeccion($_POST['cbo_seccion']);
				...
				echo ">>>>".$_POST["cdProd"];
		if ($_POST["cdProd"]=="")// REGISTRA DIRECCION
		{	
		...
		{
		//id producto
		$mantenimiento->setId_Prod($_POST['cdProd']);	
	...
	case 19:
		...
		case 20:
			...
			$intIdFila=trim($_REQUEST["id"]-1);
			$intCdConceptos=$_POST["cdConceptos$intIdFila"];
			$databaseManager = new baseDatos();
			$objMantenimiento = new mantenimiento($databaseManager);		
			
			$objMantenimiento->setConcepto($intCdConceptos);
			$bolResultado=$objMantenimiento->eliminarConceptos();
			$arrResultado=array();
			if ($bolResultado==TRUE)
			{
				$stsWhere="";
				$intTotalConsulta=$objMantenimiento->cargarTabla(1,$stsWhere);
				$_SESSION['contadorRegistro'] = $intTotalConsulta;			
				$objMantenimiento->cargarTabla(0,$stsWhere);
				$_SESSION['arrIntIdCargo'] = $objMantenimiento->getArrIntIdCargo();
				$_SESSION['arrStsNbCargo'] = $objMantenimiento->getArrStsNbCargo();
			...
			{
				$arrResultado['error']=1;
				$arrResultado['mensaje']='<li>Problemas al Eliminar</li>';
